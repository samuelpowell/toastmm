...
void TFwdSolver<T>::SetPrecon(PreconType type)
{
    ...
    {
    case PRECON_DIAG:
        precon = new TPrecon_Diag<T>;
        break;
    case PRECON_ICH:
        precon = new TPrecon_IC<T>;
        break;
    default:
        precon = new TPrecon_Null<T>;
        break;
    }
}
...
void CalcFields_engine(task_data *td)
{
    ...
    if (thdata->res)
    {
        ...
        if (err_max > thdata->res->rel_err)
            thdata->res->rel_err = err_max;
    ...
    const CVector &vec) const
{
    int n = vec.Dim();
    RVector rvec(n * 2);
    RVector rvec_r(rvec, 0, n);
    rvec_r = Re(vec);
    RVector rvec_i(rvec, n, n);
    rvec_i = Im(vec);
    return rvec;
}
    ...
    const SCVector &vec) const
{
    int n = vec.Dim();
    FVector rvec(n * 2);
    FVector rvec_r(rvec, 0, n);
    rvec_r = Re(vec);
    FVector rvec_i(rvec, n, n);
    rvec_i = Im(vec);
    return rvec;
}
...
void TFwdSolver<T>::WriteParams(ParamParser &pp)
{
    ...
    else if (solvertp == LSOLVER_ITERATIVE)
    {
        ...
        {
        case ITMETHOD_CG:
            pp.PutString("LINSOLVER", "CG");
            break;
        case ITMETHOD_BICG:
            pp.PutString("LINSOLVER", "BICG");
            break;
        case ITMETHOD_BICGSTAB:
            pp.PutString("LINSOLVER", "BICGSTAB");
            break;
        case ITMETHOD_GMRES:
            pp.PutString("LINSOLVER", "GMRES");
            break;
        case ITMETHOD_GAUSSSEIDEL:
            pp.PutString("LINSOLVER", "GAUSSSEIDEL");
            break;
        }
        ...
        {
        case PRECON_NULL:
            pp.PutString("LINSOLVER_PRECON", "NONE");
            break;
        case PRECON_DIAG:
            pp.PutString("LINSOLVER_PRECON", "DIAG");
            break;
        case PRECON_ICH:
            pp.PutString("LINSOLVER_PRECON", "ICH");
            break;
        case PRECON_DILU:
            pp.PutString("LINSOLVER_PRECON", "DILU");
            break;
        }
    }
}
