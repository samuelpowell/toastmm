...
class Mesh:
  ...
  def Jacobian(self, hraster, dphi, aphi, proj):
    ...
    Note:
      ...
      If raster is set to -1, the Jacobian is constructed directly in the mesh basis. n in
      that case is equal to the number of nodes.
    """
    return toastmod.Jacobian(self.handle, hraster, dphi, aphi, proj)

  def JacobianOptical(self,
                      hraster,
                      qvec,
                      mvec,
                      mua,
    ...
    return toastmod.JacobianOptical(self.handle, hraster, qvec.data, qvec.indptr,
...
def JacobianCW(hmesh, hraster, qvec, mvec, mua, mus, ref, solver='bicgstab', tol=1e-12):
  ...
  return toastmod.JacobianCW(hmesh, hraster, qvec.data, qvec.indptr, qvec.indices,
...
def Gradient(hmesh, hraster, qvec, mvec, mua, mus, ref, freq, data, sd):
  ...
  return toastmod.Gradient(hmesh, hraster, qvec.data, qvec.indptr, qvec.indices, mvec.data,
