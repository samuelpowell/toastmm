...
class Element:
  ...
  def Mat(self, intstr, param=None, sideidx=-1):
    ...
    Parameters:
      ...
      param:   nodal parameter array (only for integrals involving a field parameter)

    Return value:
      mat:  matrix of integral values. Each dimension of mat is of length nnd, where nnd is
            the number of nodes in the element. The number of dimensions of mat depends on 
            the integral type. 

    Notes:
      The integral types recognised by this function are given by the list below, using
      ...
      The following integrals include an integral of a function defined in the element,
      given by nodal coefficients Pi, and require the 'prm' parameter to be specified in
      the call:
      ...
      For boundary integrals ('BndF' and 'BndFF'), the integrals are performed over all
      element sides that are part of the mesh surface. Alternatively, by specifying a
      ...
      The returned matrix is of dimension n x n, where n is the number of nodes in the
      element. Nonzero entries are located at positions (i,j) where both nodes i and j
      belong to a boundary side, or to side 'sideidx', if applicable.
    """
  ...
  def ShapeFun(self, pt, frame=0):
    ...
    Notes:
      For each of the supplied evaluation points, this method returns the values of the
      shape functions associated with all emement nodes.

      Parameter 'pts' is a matrix of dimension d x np, representing a list of np
      evaluation points. d is the dimension (2 or 3).

      By default, input coordinates are interpreted in the local element frame. If
      coordinates are in the global mesh frame, set frame=1.

      The dimension of the returned matrix is nn x np, where np is the number of input
      points, and nn is the number of element nodes. Thus, fun(i,j) is the shape function
      for the j-th point and i-th node. 
      
      The order of the shape function array is defined by the local element node order.
    """
